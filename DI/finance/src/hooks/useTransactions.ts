...
export const useTransactions = () => {
    ...
    const fetchTransactions = async () => {
        try {
            ...
            // 2. Try Supabase if configured AND not guest
            if (user.id !== 'guest-user' && import.meta.env.VITE_SUPABASE_URL && import.meta.env.VITE_SUPABASE_ANON_KEY) {
    ...
    const addTransaction = async (newTransaction: NewTransaction) => {
        try {
            ...
            if (user && user.id !== 'guest-user' && import.meta.env.VITE_SUPABASE_URL) {
    ...
    return { transactions, loading, error, addTransaction, updateTransaction, deleteTransaction, refresh: fetchTransactions };
};
